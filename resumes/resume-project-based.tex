...
\centerline{
%address
$if(address)$\faicon{map-marker} $address$ \hspace{1 mm} $endif$%
%github
$if(github)$\faicon{github} \href{http://github.com/$github$}{\color{black} $github$} \hspace{1 mm} $endif$%
%twitter
$if(twitter)$\faicon{twitter} \href{http://twitter.com/$twitter$}{\color{black} $twitter$} \hspace{1 mm} $endif$%
%phone
$if(phone)$\faicon{phone} $phone$ \hspace{1 mm} $endif$%
%email
$if(email)$\faicon{envelope} \href{mailto:}{\color{black} $email$} \hspace{1 mm} $endif$%
%linkedin
$if(linkedin)$\faicon{linkedin} \href{https://www.linkedin.com/in/$linkedin$}{$linkedin$} \hspace{1 mm} $endif$%
%scholar
$if(scholar)$\faicon{google} \href{https://scholar.google.com/citations?user=$scholar$}{$scholar$} \hspace{1 mm} $endif$%
%orcid
$if(orcid)$\faicon{openid} \href{https://orcid.org/$orcid$}{$orcid$} \hspace{1 mm} $endif$%
%website
$if(www)$\faicon{home} \href{http://$www$}{$www$}\hspace{1 mm} $endif$%
}
...
#2 & \parbox[t]{0.85\textwidth}{%
      \textbf{#1}\hfill{#3}\\
      \ifx{#4\land#5}\empty\else{\small #4 \normalsize \hfill{#5}\par}\fi%
      \ifx#6\empty\else{%
        \vspace{0.5\parsep}
...
% Templates for detailed project
% Arguments: project, employer, partner, languages, skills, items
\def\detailedprojectitem#1#2#3#4#5#6{
\parbox[t]{\textwidth}{%
      \textbf{#1}\hfill{#3}\\
      ...
      \ifx#6\empty\else{%
        ...
        \begin{itemize}[leftmargin=1cm]#6\end{itemize}%
        \end{minipage}}\fi%
        \end{addmargin}
      \vspace{\parsep}}\\}
\def\detailedproject#1{\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}ll}#1\end{tabular*}}
